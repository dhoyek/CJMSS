...
(function () {
    ...
    function notify(fc, msg, level, id, timeout) {
        try {
            // Persist notifications by default; they will be cleared explicitly
    ...
    PDG.Inventory = {
        ...
        onLoad: function (executionContext) {
            ...
            // Ensure calculated values and status are populated on load
        ...
        onItemChanged: function (formContext) {
            ...
            var itemVal = attr(formContext, "pdg_itemid") && attr(formContext, "pdg_itemid").getValue();
            if (!itemVal) {
                // Clear all item-related values when item is cleared
                var fieldsToClear = [
                    "pdg_costprice","pdg_publicprice","pdg_standardcost",
                    "pdg_length","pdg_width","pdg_height",
                    "pdg_grossweight","pdg_netweight","pdg_stoneweight",
                    "pdg_reorderpoint","pdg_minimumstock",
                    "pdg_serialnumber","pdg_batchnumber","pdg_lotid",
                    "pdg_expirydate","pdg_manufacturingdate",
                    "pdg_barcodescan","pdg_barcode","pdg_qrcode",
                    "pdg_displayname"
                ];
                fieldsToClear.forEach(function (f) { setIf(formContext, f, null); });
                clear(formContext, "serial_required");
                clear(formContext, "item_error");
                clear(formContext, "item_details");
                this.recalculateQuantities(formContext);
                this.updateStatusIndicator(formContext);
                        ...
                        Xrm.WebApi.retrieveMultipleRecords("pdg_bin", binFilter)
                            .then(function (bres) {
                                if (bres.entities.length > 0) {
                                    ...
                                    scanAttr.setValue(null);
                                    clear(formContext, "barcode_not_found");
                                    // Update dependent fields/validations
                                    PDG.Inventory.updateLocationPath(formContext);
                                    PDG.Inventory.validateLocationHierarchy(formContext);
                                } else {
                                    notify(formContext, "No item/bin found for scanned code", "WARNING", "barcode_not_found", 5000);
                                }
                            });
                    }
                })
        ...
        updateDisplayName: function (formContext) {
            ...
            // Format: Item - Warehouse-Bin - SKU (no quantity)
        ...
        loadItemSnapshot: function (formContext) {
            ...
            Xrm.WebApi.retrieveRecord("pdg_inventoryitem", id, selectFields)
                .then(function (rec) {
                    ...
                    if (rec.pdg_barcode) msg += " | Barcode: " + rec.pdg_barcode;
        ...
        autoPopulateFromItem: function (formContext, itemData) {
            // Helper to safely check if a form attribute is empty
            function isEmpty(fc, name) {
                var a = attr(fc, name);
                return !a || a.getValue() === null || a.getValue() === undefined || a.getValue() === "";
            }

            // Auto-populate cost fields
            if (itemData.pdg_unitcost && isEmpty(formContext, "pdg_costprice")) {
                setIf(formContext, "pdg_costprice", itemData.pdg_unitcost);
            }
            if (itemData.pdg_publicprice && isEmpty(formContext, "pdg_publicprice")) {
                setIf(formContext, "pdg_publicprice", itemData.pdg_publicprice);
            }
            if (itemData.pdg_standardcost && isEmpty(formContext, "pdg_standardcost")) {
                setIf(formContext, "pdg_standardcost", itemData.pdg_standardcost);
            }

            // Auto-populate physical dimensions
            if (itemData.pdg_length && isEmpty(formContext, "pdg_length")) {
                setIf(formContext, "pdg_length", itemData.pdg_length);
            }
            if (itemData.pdg_width && isEmpty(formContext, "pdg_width")) {
                setIf(formContext, "pdg_width", itemData.pdg_width);
            }
            if (itemData.pdg_height && isEmpty(formContext, "pdg_height")) {
                setIf(formContext, "pdg_height", itemData.pdg_height);
            }

            // Auto-populate weights (only existing fields)
            if (itemData.pdg_grossweight && isEmpty(formContext, "pdg_grossweight")) {
                setIf(formContext, "pdg_grossweight", itemData.pdg_grossweight);
            }
            if (itemData.pdg_netweight && isEmpty(formContext, "pdg_netweight")) {
                setIf(formContext, "pdg_netweight", itemData.pdg_netweight);
            }
            if (itemData.pdg_stoneweight && isEmpty(formContext, "pdg_stoneweight")) {
            ...
            if (itemData.pdg_reorderlevel && isEmpty(formContext, "pdg_reorderpoint")) {
                setIf(formContext, "pdg_reorderpoint", itemData.pdg_reorderlevel);
            }
            if (itemData.pdg_safetystock && isEmpty(formContext, "pdg_minimumstock")) {
                setIf(formContext, "pdg_minimumstock", itemData.pdg_safetystock);
            }

            // Set condition status for hazardous materials
            if (itemData.pdg_hazardousmaterial && isEmpty(formContext, "pdg_conditionstatus")) {
                setIf(formContext, "pdg_conditionstatus", 762170002); // Requires inspection
            }

            // Populate barcode/QR fields on inventory record if present
            if (attr(formContext, "pdg_barcode")) {
                setIf(formContext, "pdg_barcode", itemData.pdg_barcode || itemData.pdg_sku || "");
            }
            if (attr(formContext, "pdg_qrcode")) {
                setIf(formContext, "pdg_qrcode", itemData.pdg_qrcode || "");
            }
        },
        ...
        recalculateCosts: function (formContext) {
            ...
            // Weighted Average Cost (simple per-record: totalValue / qty, fallback to cost)
            var wac = qty > 0 ? (totalValue / qty) : cost;
            setIf(formContext, "pdg_weightedaveragecost", wac);
        ...
        validateSerialNumbers: function (formContext) {
            ...
            // Lot lookup can replace batch number
            var lotLookup = attr(formContext, "pdg_lotid") && attr(formContext, "pdg_lotid").getValue();
            var lotSelected = !!getLookupId(lotLookup);
            ...
            Xrm.WebApi.retrieveRecord("pdg_inventoryitem", itemId, "?$select=pdg_serialcontrolled,pdg_expirytracking")
                .then(function (itemData) {
                    ...
                    if (requiresSerial && !serialNumber && !batchNumber && !lotSelected) {
        ...
        validateLocationHierarchy: function (formContext) {
            ...
            if (bin) {
                ...
                Xrm.WebApi.retrieveRecord("pdg_bin", binId, "?$select=_pdg_warehouseid_value")
                    .then(function (binData) {
                        var binWh = (binData._pdg_warehouseid_value || "").toString().replace(/[{}]/g, "").toLowerCase();
                        var wh = (warehouseId || "").toString().replace(/[{}]/g, "").toLowerCase();
                        if (binWh !== wh) {
                ...
                // No bin selected; clear any previous error
                clear(formContext, "location_hierarchy");
            }
