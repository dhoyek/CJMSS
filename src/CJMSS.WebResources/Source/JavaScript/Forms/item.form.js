...
PDG.Item = {
    ...
    onLoad: function (executionContext) {
        ...
        // Stock notifications are evaluated after inventory data loads
    ...
    loadInventoryDetailsSimple: function (formContext) {
        ...
        Xrm.WebApi.retrieveMultipleRecords("pdg_inventory",
            ...
            function success(result) {
                ...
                // Do not write rollup fields (handled server-side):
                // pdg_totalquantityonhand, pdg_totalquantityonhand_date
    ...
    updateQuantityFields: function (formContext, inventoryData) {
        try {
            ...
            // Do not write rollup fields (server-calculated):
            // - pdg_totalquantityonhand
            // - pdg_totalquantityonhand_date
            // - pdg_totalvalue
    ...
    // getWarehouseInventory removed (no longer used)

    // ========= Cascading Lookup Filters =========

    // filterFamily removed (no longer used)
    ...
    calculateTotalValueWithCurrency: function (formContext) {
        // Calculated server-side (rollup/calculated column). Keep only for local use if needed.
        var qA = formContext.getAttribute("pdg_quantityonhand");
        ...
        formContext.PDG_Totals = formContext.PDG_Totals || {};
        formContext.PDG_Totals.totalValue = totalValue;
        formContext.PDG_Totals.totalValueBase = totalValueBase;
    },
    ...
    refreshDashboards: function (formContext) {
        try {
            ...
            var self = this;
            this.loadTransactionData(formContext).then(function (tx) {
                var velocity = tx.metrics.velocity || 0;
                var turnover = tx.metrics.turnover || 0;
                var averageAge = tx.metrics.averageAge || 0;

                // Stock Status Dashboard
                self._postToWR(formContext, "WebResource_StockStatusDashboard", "updateSummaryData", {
                    totalQuantityOnHand: onhand,
                    totalValue: totalValue,
                ...
                // Alerts Panel (single pass, same source as notifications)
                self._fcLast = formContext;
                var alerts = self.computeAlerts(onhand, reorder, safety);
                self._postToWR(formContext, "WebResource_AlertsPanel", "updateAlertsData", { alerts: alerts, timestamp: new Date().toLocaleString() }, "data");

                // Inventory Analytics with movement history and KPIs
                ...
                self._postToWR(formContext, "WebResource_TransactionTimeline", "updateTransactionData", { transactions: tx.timeline }, "payload");
            }).catch(function (e) {
                console.warn("refreshDashboards tx error", e);
            });
        } catch (e) {
    ...
    computeAlerts: function (onhand, reorder, safety) {
        var alerts = [];
        var fc = (this && this.WR_IDS) ? this._fcLast || null : null;

        if (onhand <= 0) {
            alerts.push({ type: "critical", priority: 1, icon: "⚠️", title: "Out of Stock", message: "No inventory available." });
        } else {
            if (safety > 0 && onhand <= safety) {
                alerts.push({ type: "critical", priority: 1, icon: "⛔", title: "Below Safety Stock", message: "Immediate replenishment required." });
            } else if (reorder > 0 && onhand <= reorder) {
                alerts.push({ type: "warning", priority: 2, icon: "⚠️", title: "Reorder Point Reached", message: "Plan purchase or transfer." });
            }

            // Overstock indicator (parity with notification section)
            try {
                var targetAttr = Xrm.Page ? Xrm.Page.getAttribute("pdg_stocktarget") : (fc && fc.getAttribute && fc.getAttribute("pdg_stocktarget"));
                var target = Number((targetAttr && targetAttr.getValue()) || 0);
            ...
            // Margin health
            try {
                var pp = Number(((fc && fc.getAttribute("pdg_publicprice")) && fc.getAttribute("pdg_publicprice").getValue()) || 0);
                var uc = Number(((fc && fc.getAttribute("pdg_unitcost")) && fc.getAttribute("pdg_unitcost").getValue()) || 0);
                if (pp > 0 && uc > 0) {
                    var margin = ((pp - uc) / pp) * 100;
    ...
    loadTransactionData: function (formContext) {
        try {
            var itemId = (formContext.data && formContext.data.entity && formContext.data.entity.getId && formContext.data.entity.getId()) || null;
            if (!itemId) return Promise.resolve({ history: [], timeline: [], metrics: { velocity: 0, turnover: 0, averageAge: 0 } });
            itemId = itemId.replace(/[{}]/g, '');
            ...
            return Xrm.WebApi.retrieveMultipleRecords("pdg_inventorytransaction", filter).then(function (res) {
                ...
                var buckets = {};
                var totalOutbound30 = 0;
                var totalInbound30 = 0;
                var inboundAgeSum = 0; // qty-weighted
                var inboundQtySum = 0;

                function mapType(e) {
                    var t = e["pdg_transactiontype@OData.Community.Display.V1.FormattedValue"] || "";
                    t = t.toLowerCase();
                    if (t.indexOf('in') === 0) return 'inbound';
                    if (t.indexOf('out') === 0) return 'outbound';
                    if (t.indexOf('adjust') === 0) return 'adjustment';
                    if (t.indexOf('transfer') === 0) return 'transfer';
                    if (t.indexOf('count') === 0) return 'count';
                    return 'adjustment';
                }

                var timeline = txs.slice(0, 50).map(function (e) {
                    var d = e.pdg_transactiondate ? new Date(e.pdg_transactiondate) : now;
                    var type = mapType(e);
                    var qty = Number(e.pdg_quantity || 0);
                    var totalValue = Number(e.pdg_totalcost || 0);
                    var unitPrice = Number(e.pdg_unitcost || 0);
                    var warehouseName = e["_pdg_towarehouseid_value@OData.Community.Display.V1.FormattedValue"] || e["_pdg_fromwarehouseid_value@OData.Community.Display.V1.FormattedValue"] || "N/A";
                    return {
                        date: d.toISOString(),
                        type: type,
                        quantity: qty,
                        ...
                        reference: e.pdg_referencenumber || '',
                        notes: e.pdg_remarks || ''
                    };
                });
                ...
                // Build ordered history last 30 days
                var history = Object.keys(buckets).sort().map(function (k) {
                    var b = buckets[k];
                    return { date: k, inbound: b.inbound, outbound: b.outbound, balance: Math.max(0, b.inbound - b.outbound) };
                });

                var onhandAttr = formContext.getAttribute("pdg_totalquantityonhand") || formContext.getAttribute("pdg_quantityonhand");
                var onhand = Number((onhandAttr && onhandAttr.getValue()) || 0);
                var velocity = totalInbound30 + totalOutbound30; // units/month approximation
                var turnover = onhand > 0 ? (12 * totalOutbound30 / onhand) : 0; // outflow-based
                var averageAge = inboundQtySum > 0 ? Math.round(inboundAgeSum / inboundQtySum) : 0;

                return {
                    history: history,
                    timeline: timeline,
                    metrics: { velocity: velocity, turnover: turnover, averageAge: Math.round(averageAge) }
                };
            });
        } catch (e) {
            return Promise.resolve({ history: [], timeline: [], metrics: { velocity: 0, turnover: 0, averageAge: 0 } });
        }
    },
};
